...
export const ActionDialog: React.FC<ActionDialogProps> = ({ row }) => {
  ...
  const columns = [
    ...
    { header: "Discovery Telescope", value: row.original.elements.disc_telescope },
    { header: "Orbital Period [days]", value: row.original.elements.period },
    { header: "Orbit Semi-Major Axis [au]", value: row.original.elements.semiMajorAxis },
    ...
    { header: "Epoch of Periastron [days]", value: row.original.elements.epoch },
    { header: "Argument of Periastron [deg]", value: row.original.elements.argOfPeriapsis },
    { header: "Stellar Radius [Solar Radius]", value: row.original.elements.star_radius },
    { header: "Distance [pc]", value: row.original.elements.star_distance },
    { header: "ESMax", value: row.original.elements.esmax },
    { header: "Is Characterizable?", value: row.original.elements.isCharacterizable ? "Yes" : "No" },
  ];
      ...
      <DropdownMenuContent align="end">
          ...
          <Dialog open={open3DViewer} onOpenChange={setOpen3DViewer}>
            ...
            <DialogContent className="w-full h-screen p-2">
              ...
              <div className="w-full h-screen p-3 flex items-center justify-center">
                <iframe id="orbitViewerContainer" className="w-[450px] h-[350px]" src="/orbits/orbit.html"></iframe>
              </div>
        ...
        <DropdownMenuItem asChild>
          <Dialog open={openProperties} onOpenChange={setOpenProperties}>
            ...
            <DialogContent className="w-[2280px] h-[950px] p-2">
              <DialogHeader>
                ...
                <DialogDescription className="flex items-center justify-center">
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead>
                      <tr>
                        <th className="px-3 py-1 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Property</th>
                        <th className="px-3 py-1 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Value</th>
                      </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-gray-200">
                      {columns.map((col, index) => (
                        <tr key={index}>
                          <td className="px-3 py-1 whitespace-nowrap text-sm text-gray-900">{col.header}</td>
                          <td className="px-3 py-1 whitespace-nowrap text-sm text-gray-900">{col.value}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
